#!/usr/bin/env python3
"""Evaluate deterministic RM65+4C2 reach control on multiple reachable targets."""

from __future__ import annotations

import argparse
import json
from pathlib import Path


parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument("--usd", type=Path, required=True)
parser.add_argument("--urdf", type=Path, required=True)
parser.add_argument("--description", type=Path, required=True)
parser.add_argument("--output", type=Path, required=True)
parser.add_argument("--trials", type=int, default=12)
parser.add_argument("--seed", type=int, default=7)
parser.add_argument("--trajectory-steps", type=int, default=120)
parser.add_argument("--settle-steps", type=int, default=60)
args = parser.parse_args()

from isaacsim import SimulationApp  # noqa: E402


simulation_app = SimulationApp({"headless": True})

import numpy as np  # noqa: E402
from isaacsim.core.api import World  # noqa: E402
from isaacsim.core.api.objects import VisualCuboid  # noqa: E402
from isaacsim.core.prims import SingleArticulation  # noqa: E402
from isaacsim.core.utils.rotations import rot_matrix_to_quat  # noqa: E402
from isaacsim.core.utils.stage import add_reference_to_stage  # noqa: E402
from isaacsim.core.utils.types import ArticulationAction  # noqa: E402
from isaacsim.robot_motion.motion_generation import (  # noqa: E402
    ArticulationKinematicsSolver,
    LulaKinematicsSolver,
)


ARM_JOINTS = [f"joint_{index}" for index in range(1, 7)]
END_EFFECTOR_FRAME = "link_6"
TARGET_LOWER = np.array([-0.40, -1.00, 0.85, -0.35, 0.40, -0.50])
TARGET_UPPER = np.array([0.40, -0.55, 1.30, 0.35, 0.85, 0.50])


def rotation_error_radians(actual: np.ndarray, target: np.ndarray) -> float:
    relative = actual.T @ target
    cosine = np.clip((np.trace(relative) - 1.0) / 2.0, -1.0, 1.0)
    return float(np.arccos(cosine))


def main() -> int:
    usd = args.usd.expanduser().resolve()
    urdf = args.urdf.expanduser().resolve()
    description = args.description.expanduser().resolve()
    output = args.output.expanduser().resolve()
    missing = [str(path) for path in (usd, urdf, description) if not path.is_file()]
    if missing:
        raise FileNotFoundError(f"missing required files: {missing}")
    if args.trials < 1 or args.trajectory_steps < 2 or args.settle_steps < 1:
        raise ValueError("invalid trial or step count")

    rng = np.random.default_rng(args.seed)
    world = World(stage_units_in_meters=1.0)
    world.get_physics_context().set_gravity(0.0)
    asset_prim = "/World/RM65_4C2"
    add_reference_to_stage(usd_path=str(usd), prim_path=asset_prim)
    robot = world.scene.add(
        SingleArticulation(prim_path=f"{asset_prim}/root_joint", name="rm65_4c2")
    )
    cube = world.scene.add(
        VisualCuboid(
            prim_path="/World/TargetCube",
            name="target_cube",
            position=np.array([0.4, 0.0, 0.05]),
            scale=np.array([0.05, 0.05, 0.05]),
            color=np.array([0.85, 0.10, 0.08]),
        )
    )
    world.reset()

    dof_names = list(robot.dof_names)
    arm_indices = [dof_names.index(name) for name in ARM_JOINTS]
    start_q = np.array([0.0, -0.5, 1.0, 0.0, 0.5, 0.0], dtype=np.float64)
    lula = LulaKinematicsSolver(robot_description_path=str(description), urdf_path=str(urdf))
    articulation_ik = ArticulationKinematicsSolver(
        robot_articulation=robot,
        kinematics_solver=lula,
        end_effector_frame_name=END_EFFECTOR_FRAME,
    )
    controller = robot.get_articulation_controller()
    trial_reports = []

    for trial_index in range(args.trials):
        source_q = rng.uniform(TARGET_LOWER, TARGET_UPPER)
        target_position, target_rotation = lula.compute_forward_kinematics(END_EFFECTOR_FRAME, source_q)
        target_orientation = rot_matrix_to_quat(target_rotation)
        cube_position = target_position.copy()
        cube_position[2] -= 0.10
        cube.set_world_pose(position=cube_position)

        robot.set_joint_positions(start_q, joint_indices=arm_indices)
        world.step(render=False)
        initial_position, _ = articulation_ik.compute_end_effector_pose()
        action, ik_success = articulation_ik.compute_inverse_kinematics(
            target_position=target_position,
            target_orientation=target_orientation,
            position_tolerance=1e-4,
            orientation_tolerance=1e-3,
        )
        if not ik_success or action.joint_positions is None:
            trial_reports.append(
                {
                    "trial": trial_index,
                    "status": "fail",
                    "reason": "ik_failed",
                    "source_target_joint_position_rad": source_q.tolist(),
                }
            )
            continue

        solved_q = np.asarray(action.joint_positions, dtype=np.float64)
        command_history = [start_q.copy()]
        for step in range(args.trajectory_steps):
            progress = (step + 1) / args.trajectory_steps
            smooth_progress = 3.0 * progress**2 - 2.0 * progress**3
            command = start_q + smooth_progress * (solved_q - start_q)
            controller.apply_action(
                ArticulationAction(joint_positions=command, joint_indices=np.asarray(arm_indices))
            )
            command_history.append(command.copy())
            world.step(render=False)
        for _ in range(args.settle_steps):
            controller.apply_action(
                ArticulationAction(joint_positions=solved_q, joint_indices=np.asarray(arm_indices))
            )
            world.step(render=False)

        actual_position, actual_rotation = articulation_ik.compute_end_effector_pose()
        actual_q = np.asarray(robot.get_joint_positions(joint_indices=arm_indices), dtype=np.float64)
        position_error = float(np.linalg.norm(actual_position - target_position))
        rotation_error = rotation_error_radians(actual_rotation, target_rotation)
        joint_error = float(np.max(np.abs(actual_q - solved_q)))
        initial_error = float(np.linalg.norm(initial_position - target_position))
        max_step = float(np.max(np.abs(np.diff(np.asarray(command_history), axis=0))))
        passed = (
            position_error < 0.01
            and rotation_error < 0.05
            and joint_error < 0.03
            and max_step < 0.05
            and initial_error > position_error + 0.03
        )
        trial_reports.append(
            {
                "trial": trial_index,
                "status": "pass" if passed else "fail",
                "source_target_joint_position_rad": source_q.tolist(),
                "solved_joint_position_rad": solved_q.tolist(),
                "target_position_m": target_position.tolist(),
                "initial_position_error_m": initial_error,
                "final_position_error_m": position_error,
                "final_rotation_error_rad": rotation_error,
                "final_joint_error_rad": joint_error,
                "maximum_command_step_rad": max_step,
            }
        )

    passed_trials = sum(item["status"] == "pass" for item in trial_reports)
    completed = [item for item in trial_reports if "final_position_error_m" in item]
    success_rate = passed_trials / args.trials
    suite_passed = success_rate >= 0.95
    report = {
        "status": "pass" if suite_passed else "fail",
        "simulation_only": True,
        "task": "multi-target deterministic pre-grasp reach",
        "seed": args.seed,
        "trial_count": args.trials,
        "passed_trials": passed_trials,
        "success_rate": success_rate,
        "gravity_enabled": False,
        "target_joint_sampling_lower_rad": TARGET_LOWER.tolist(),
        "target_joint_sampling_upper_rad": TARGET_UPPER.tolist(),
        "worst_final_position_error_m": max(
            (item["final_position_error_m"] for item in completed), default=None
        ),
        "worst_final_rotation_error_rad": max(
            (item["final_rotation_error_rad"] for item in completed), default=None
        ),
        "worst_final_joint_error_rad": max(
            (item["final_joint_error_rad"] for item in completed), default=None
        ),
        "maximum_command_step_rad": max(
            (item["maximum_command_step_rad"] for item in completed), default=None
        ),
        "criteria": {
            "success_rate_gte": 0.95,
            "per_trial_position_error_m_lt": 0.01,
            "per_trial_rotation_error_rad_lt": 0.05,
            "per_trial_joint_error_rad_lt": 0.03,
            "per_trial_command_step_rad_lt": 0.05,
        },
        "trials": trial_reports,
    }
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({key: value for key, value in report.items() if key != "trials"}, indent=2))
    print(f"RM65_REACH_ROBUSTNESS={'PASS' if suite_passed else 'FAIL'}", flush=True)
    return 0 if suite_passed else 1


try:
    exit_code = main()
finally:
    simulation_app.close()

raise SystemExit(exit_code)
