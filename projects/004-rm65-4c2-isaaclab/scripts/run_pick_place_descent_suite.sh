...
for angle in "${angles[@]}"; do
  label="${angle/./p}"
  echo "PLACE_DESCENT_START=$angle"
  if ! "$isaaclab_root/isaaclab.sh" -p scripts/run_pick_place_baseline.py \
    ...
    --output "outputs/natural_place_descent_${label}.json" \
    ...
    --target-support-mode rotated_strip \
    --target-collision-enable-stage after_place_descent \
    --place-descent \
    --place-descent-distance-m 0.10 \
    --unassisted-release \
    --headless \
    > "outputs/natural_place_descent_${label}.log" 2>&1; then
    trial_failed=1
  fi
  echo "PLACE_DESCENT_DONE=$angle"
done

python3 scripts/summarize_pick_place_descent.py \
  --output results/natural_place_descent_robustness.json || trial_failed=1
