...
0.6、0.8、1.0 rad 三次全部通过：最小物体抬升 `39.07 mm`，末端实际下降约 `100.06 mm`，最大下降关节误差 `0.01194 rad`，最大最终落点误差 `9.48 mm`。释放阶段没有直接改写方块位姿或速度。该结果仍是脚本专家与仿真开发基线：运输期间关闭机械臂重力，六个活动指节隔离重力，使用未经实物标定的宽碰撞垫和旋转窄支座，并在下降完成后才启用目标支座碰撞。下一步应恢复目标碰撞完成整段下降、校准实物参数，再把相机观测与专家动作整合成数据采集环境。
