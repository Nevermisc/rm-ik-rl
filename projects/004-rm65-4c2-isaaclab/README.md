...
- 修正抬升轨迹，使末端沿世界 z 方向上升 40 mm；随后加入分段笛卡尔下降、随转运角旋转的窄目标支座和无位置/速度注入的自然释放。0.6、0.8、1.0 rad 三次均完成自然重力抓取、抬升、转运、下降、落台和撤离，最大最终误差 `9.48 mm`；
- 发现旧场景把 RM65 安装面错误地留在世界 `z=0`，而源支撑面位于 `z=0.733 m`，导致顶部抓取腕部目标超出工作半径；加入显式 `--robot-base-z-m` 和世界/基座坐标换算后，在 `z=0.65 m` 安装面完成严格顶部抓取；
- 顶部抓取完整任务已在机械臂、六个活动夹指和方块全部启用重力时通过：从 10 cm 外接近，抬升 `39.57 mm`，转运约 17 cm，下降 120 mm，自然释放并撤离；单次最终误差 `17.56 mm`，释放后漂移 `1.24 mm`，没有直接改写方块位姿或速度；
- 修复下降 IK 在等价关节分支之间跳变的问题：先在未旋转基座坐标中求连续下降轨迹，再只旋转关节 1；最大相邻路点变化由 `3.51 rad` 降至 `0.04031 rad`。0.6、0.8、1.0 rad 三组全重力完整任务全部通过；
...
- 加入失败即关闭的策略执行门：只有 RM65-B + 4C2 的 π0.5 checkpoint、归一化统计和动作语义证据齐全时才允许进入仿真；真机还必须额外满足至少 20 次仿真评测、成功率不低于 80%、急停、低速与空工作区验收；
- DROID checkpoint 的输出只做接口 dry-run，不发送给仿真或真机。
- 已实现 20 Hz 脚本专家 episode 记录器，把六轴实际状态、4C2 归一化状态、下一步绝对目标、方块位姿、语言指令和任务阶段同步保存并严格校验；外部与腕部 RGB 已接入同一采样点，代码仍需在实验室 IsaacLab 中完成双相机全任务回归后才能确认训练就绪。
...
| 高位直接辅助释放历史复测 | 1/3；用于定位释放问题 |
| 分段下降 + 自然释放状态机 | 3/3；最大最终误差 `0.00948 m` |
| 放置下降最大关节误差 | `0.01194 rad` |
| 顶部抓取全重力完整任务 | `3/3`，`unassisted_full_task_complete=true` |
| 顶部抓取最小抬升 / 最大最终误差 / 最大释放漂移 | `39.57 / 17.56 / 2.35 mm` |
| 顶部抓取下降 IK 最大相邻路点变化 | `0.04031 rad` |
| USD 刚体 / 启用碰撞体 | `16 / 16` |
| 外部图红色目标像素 | 966 |
| 腕部图红色目标像素 | 1256 |
...
从脚本专家生成 π0.5 数据的格式、帧语义和运行方法见 [RM65 专家示教数据说明](docs/RM65_EXPERT_DATASET_ZH.md)。

更换实验室 Linux 电脑前的清点、备份、恢复和逐层验收见 [实验室电脑迁移清单](docs/LAB_COMPUTER_MIGRATION_ZH.md)。
...
该历史套件让方块从一开始就承受重力，并从 10 cm 外动态接近。三次都稳定抬升 39.07 mm，但高位释放只有 0.8 rad 完成验收；0.6 rad 因落台后漂移 43.09 mm 失败，1.0 rad 的最终误差为 52.02 mm。直接去掉释放辅助时，方块会卡在指间或随夹爪离开。

当前推荐的脚本先下降 100 mm，再张开夹爪并自然落台：

```bash
bash scripts/run_pick_place_descent_suite.sh
```

0.6、0.8、1.0 rad 三次全部通过：最小物体抬升 `39.07 mm`，末端实际下降约 `100.06 mm`，最大下降关节误差 `0.01194 rad`，最大最终落点误差 `9.48 mm`。释放阶段没有直接改写方块位姿或速度。该结果仍是脚本专家与仿真开发基线：运输期间关闭机械臂重力，六个活动指节隔离重力，使用未经实物标定的宽碰撞垫和旋转窄支座，并在下降完成后才启用目标支座碰撞。当前抓取时 `link_6` 位于支撑面下方，不能直接用于普通桌面任务；下一步应设计腕部始终位于桌面上方的抓取姿态，让目标碰撞全程启用，再校准实物参数并整合数据采集环境。

上面保留的是历史侧抓基线。当前推荐运行安装面修正后的顶部抓取全重力套件：

```bash
bash scripts/run_top_down_full_gravity_suite.sh
```

该套件把 RM65 安装面设为世界 `z=0.65 m`，源支撑面位于安装面上方 `83 mm`。Lula 仍在机器人基座坐标系中求解，场景物体继续使用世界坐标；脚本显式完成两者换算。机械臂、六个活动夹指和方块全部启用重力，目标宽平台在转运后启用碰撞，夹爪打开时不注入方块位姿或速度。控制器仍是脚本专家，使用未经实物标定的宽碰撞垫，也没有使用 π0.5。

0.6、0.8、1.0 rad 三次结果均通过：最小抬升 `39.57 mm`，最大最终误差 `17.56 mm`，最大释放后漂移 `2.35 mm`。开发过程中 0.7 和 1.0 rad 曾因 Lula 在等价 IK 分支之间跳变而失败；下降轨迹现先在未旋转基座坐标中求解，再把同一连续解绕关节 1 旋转到目标方位。修复后最大相邻关节命令为 `0.04031 rad`。
...
早期通用 articulation 冒烟测试中，4C2 的六个活动指节在 PhysX 重力下出现非物理大扭矩，因此历史基线曾隔离这六个连杆的重力。当前顶部抓取任务使用显式软件耦合、高增益驱动和有限速度轨迹，已经在六个活动夹指全部启用重力时完成一次完整任务。这个结果消除了当前任务对“关闭夹指重力”的依赖，但质量、惯量、传动与碰撞垫仍未按实物标定；历史失败证据仍保留在 `results/diagnostic_*.json`。
