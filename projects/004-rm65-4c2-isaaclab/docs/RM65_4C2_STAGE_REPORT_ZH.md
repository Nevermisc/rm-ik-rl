...
随后把一个关闭重力的 `25×60×25 mm` 悬浮测试块放在指尖内侧，移除桌面，并为六个活动指节分别建立接触传感器。六个夹爪关节缓慢闭合到 `0.55 rad`，过程没有 NaN/Inf，最终两组代理间距为 `25.56 mm` 和 `44.62 mm`。测试块移动了 `0.03671 m`，但诊断图显示初始摆位与复杂 STL 碰撞近似存在穿插；这段位移主要是碰撞解算器消除重叠的结果。按测试块路径过滤的接触力仍为 0 N，因此不能确认稳定夹持。继续移动机械臂时，当前 PhysX 进程在原生代码中退出；脚本默认不执行该路径，只有显式加入 `--attempt-lift` 才会复现。

USD 物理清单还确认组合资产包含 16 个刚体和 16 个启用碰撞体，4C2 的 9 个 link 均有启用的碰撞几何。因此当前问题已经缩小到接触报告、碰撞近似与关节动力学的组合，而不是“导入后完全没有碰撞体”。
...
1. 为 4C2 建立简化凸碰撞体，生成无穿插的物体初始位姿，并让过滤接触力正常上报；
2. 在 Isaac Lab 为 RM65 建立一个明确的单任务场景，例如“抓起红色方块并放入托盘”；
3. 写一个不依赖 π0.5 的专家控制器，先证明 RM65 和 4C2 在物理层可以完成任务；
4. 固定外部和腕部相机的安装参数；
5. 记录每个控制时刻的两张图片、六轴关节角、夹爪状态、七维目标动作和语言指令；
