...
脚本加入六个活动指节对方块的过滤接触传感器。悬空参考姿态在 `tool_l_2/tool_r_2` 得到 `0.09164/0.07518 N`。自然重力下，对抓取目标施加世界 x 方向 −40 mm 修正后，同一对连杆得到 `0.05346/0.05835 N`，确认双侧接触。把夹爪 effort、stiffness、damping 和闭合目标分别提高到 `40`、`250`、`25` 和 `0.75 rad` 后，接触力提高到 `0.07181/0.07275 N`，但 30 g 方块在抬升开始后仍脱落。

这一步的准确结论是“自然重力双侧接触通过，持续承重失败”。失败后脚本现在会在 `failure_stage=lift` 立即停止，不再继续转移和释放。下一次迭代应测量真实 4C2 指面尺寸和摩擦材料，重新标定当前经验碰撞垫，并设计与实际桌面和物体高度一致的抓取方式；在该物理基线通过以前，不把 π0.5 动作接入 RM65 仿真执行器。
...
1. 用真实夹爪尺寸和材料校准碰撞垫，并让自然重力双侧接触能够持续承重；
2. 用碰撞感知专家控制器替换方块重力辅助和释放分离辅助，把接近、闭合、抬升、移动、放置和松开完整跑通；
