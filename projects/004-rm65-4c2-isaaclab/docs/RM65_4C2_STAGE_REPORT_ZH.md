...
最初在转运高度直接张开夹爪时，方块仍被夹爪夹住；即使夹爪张开后向上撤离 80 mm，方块也被一起带到约 `z=0.839 m`。带 80 mm 向下分离和 0.10 m/s 初速度的三角度复测也只有 1/3 通过，说明需要先完成放置下降。

第一次把机械臂直接插值到低位 IK 解时发生严重碰撞发散。改为每 10 mm 求解一个保持末端方向的笛卡尔路点后，宽目标平台仍让末端只下降约 40 mm，关节误差达到 `0.186 rad`。把目标台改成随转运角旋转的窄支座后仍会在下降途中接触夹爪结构。最后让窄支座在 100 mm 分段下降完成后启用碰撞，再张开夹爪并等待自然沉降，方块不再需要位姿或速度注入。

0.6、0.8、1.0 rad 三次完整复测全部通过。最小方块抬升为 `39.07 mm`，末端实际下降约 `100.06 mm`，最大下降关节误差为 `0.01194 rad`，最大最终落点误差为 `9.48 mm`，释放后最大漂移只有 `1.49×10⁻⁸ m`。当前准确结论是“脚本专家的自然重力动态抓取、抬升、转运、下降、自然释放和撤离已 3/3 跑通”。

它仍不是无辅助物理闭环：运输期间关闭机械臂重力，六个活动指节隔离重力，宽碰撞垫未经实物标定，目标使用专门的旋转窄支座，并在下降完成后才启用支座碰撞。抓取闭合时 `link_6` 的 z 约为 `0.6463 m`，源支座顶面为 `0.7330 m`，腕部实际处在支撑面下方；这解释了宽平台为什么必然挡住当前放置轨迹，也说明该姿态不能直接迁移到普通桌面。控制器是脚本专家，`pi05_used=false`，没有向真机发送命令。
...
侧抓基线虽然完成了任务，但闭合时 `link_6` 低于源支撑面，不适合普通桌面。为此先根据已经校准的“腕部到方块”局部向量构造严格顶部抓取：夹爪开合轴保持水平，腕部到方块的方向映射到世界 `-z`，并保持局部距离不变。纯 NumPy 几何测试确认旋转矩阵正交、行列式为 1、局部偏移保持不变。

第一次在旧场景中求解时，yaw 为 0 和 ±90° 均失败；15°、30°、45°、60°、70° 倾斜也失败；严格顶部姿态使用 256 个确定性随机关节种子仍无解。连续插值表明原侧抓姿态在 10% 旋转以内尚可求解，15% 已失去精确解，而且腕部仍低于桌面。这些失败排除了单一初始种子问题。

检查坐标后发现，脚本把 RM65 基座留在世界 `z=0`，源支撑顶面却在 `z=0.733 m`。严格顶部目标的腕部高度约为 `0.873 m`，已经超过 RM65 相对基座的有效工作半径。原侧抓之所以能运行，是夹爪从支撑面下方伸向方块；这正是宽桌面会挡住腕部的原因。

脚本现加入 `--robot-base-z-m`。场景物体继续用世界坐标，Lula 的目标先减去机器人基座平移，转换到基座坐标；机器人 articulation 同时按相同高度生成。把安装面设为 `z=0.65 m` 后，严格顶部姿态由原始种子直接求解成功，10 个从 10 cm 外开始的接近路点全部完成。闭合峰值左右接触力为 `3.245/3.187 N`，最近窗口均值为 `0.984/0.999 N`。

随后执行完整任务：机械臂、六个活动夹指和方块全部启用重力；从 10 cm 外接近，笛卡尔竖直抬升 40 mm，关节 1 转动 0.8 rad，向下分 12 段移动 120 mm，在宽目标平台上自然释放，再反向撤离。目标平台从转运完成后就启用碰撞。没有初始化到抓取位，没有跳过接近碰撞，也没有直接写入方块位姿或速度。

单次机器可读结果为：方块抬升 `39.568 mm`，最终目标误差 `17.557 mm`，释放后漂移 `1.242 mm`，接近/抬升最大关节误差为 `0.01290/0.01284 rad`，全部状态为有限值，`unassisted_full_task_complete=true`。这仍是脚本专家和经验宽碰撞垫，不是 π0.5 闭环，也不代表真实硬件参数已经标定。

第一次三转角复测中，0.6 和 0.8 rad 通过，1.0 rad 在撤离时把方块带离平台。把下降速度减半没有改善。加入仅运动学诊断后定位到真正原因：0.7 rad 的第 12 个下降路点让关节 3 从 `-1.619` 跳到 `+1.621 rad`，1.0 rad 的第 4 个下降路点让关节 5 从 `+1.774` 跳到 `-1.736 rad`；最大单步分别为 `3.239/3.510 rad`。Lula 给出的每个姿态本身都满足误差，但整条关节轨迹并不连续。

由于转运只改变基座关节 1，竖直下降轨迹具有旋转对称性。修复后的算法先在未旋转的抓取方位求一次连续下降 IK，再给每个下降路点的关节 1 增加转运角。0.7 和 1.0 rad 的最大相邻路点变化都降为 `0.04031 rad`。最终 0.6、0.8、1.0 rad 三组全重力完整任务全部通过：最小抬升 `39.568 mm`，最大最终误差 `17.557 mm`，最大释放漂移 `2.354 mm`，`unassisted_full_task_complete=true`。
...
1. **已完成仿真基线**：修正机器人安装高度，建立桌面上方严格顶部抓取，并在机械臂与夹指全重力、宽目标平台碰撞启用的条件下完成完整任务；
2. 用真实夹爪尺寸、质量、惯量和材料校准碰撞垫，并对方块初始位置、质量和摩擦做扰动评测；
3. 降低放置阶段约 `0.105 rad` 的最大关节跟踪误差，确认专家轨迹适合记录为示范；
4. 校准腕部相机外参，把双相机采集、状态读取和动作执行合并进同一个固定步长 Isaac Lab 环境；
5. 用已经通过的专家控制器记录每个控制时刻的两张图片、六轴关节角、夹爪状态、七维目标动作和语言指令；
6. 转换为 OpenPI 使用的小型 LeRobot 数据集；
7. 新建 RM65 专用输入/输出 transform、归一化统计和训练配置；
8. 从 π0.5 checkpoint 微调；
9. 在多个方块位置、光照和语言表达下做闭环仿真评测；
10. 加入关节限位、单步变化限制、碰撞和工作空间保护；
11. 最后才连接真实 RM65，在有人值守、低速和空工作区条件下测试。
...
> 为 RM65 + 4C2 构建自然重力抓取脚本专家，修正笛卡尔抬升与分段放置轨迹并增加接触力时序诊断，实现从 10 cm 外接近、双侧夹持、39 mm 抬升、约 17 cm 转运、100 mm 下降和自然释放；在三个转运角下完成 3/3 仿真复测，最大落点误差 9.48 mm。
