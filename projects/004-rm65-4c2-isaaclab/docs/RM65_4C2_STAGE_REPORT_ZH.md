...
它仍不是无辅助物理闭环：运输期间关闭机械臂重力，六个活动指节隔离重力，宽碰撞垫未经实物标定，目标使用专门的旋转窄支座，并在下降完成后才启用支座碰撞。控制器是脚本专家，`pi05_used=false`，没有向真机发送命令。
...
1. 保持目标支座全程碰撞启用，调整放置路径或支座几何，使 100 mm 下降不依赖延迟碰撞；
